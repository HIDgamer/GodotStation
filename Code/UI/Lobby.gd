...
# Lobby - Displays video, music, and art media during the pre-game lobby phase.
...
func _ready() -> void:
	...
	# Load initial video for all peers if set
	if GameManager.CurrentVideoUid != "":
		load_video(GameManager.CurrentVideoUid)
	else:
		if multiplayer.is_server():
			var random_screensaver = _default_screensavers[randi() % _default_screensavers.size()]
...
func _load_music(path: String, loops: int = 1, volume: float = 0.5) -> void:
	...
	if path.begins_with("res://"):
		...
		if stream:
			...
			# Set up timer to stop after loops * length
...
func _on_game_started() -> void:
	# Stop admin videos when game starts, but keep music
	if video_player.is_playing():
		video_player.stop()
