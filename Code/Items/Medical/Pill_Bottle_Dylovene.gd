...
func _ready():
	...
	# Item properties
	item_name = "Pill Bottle (Dylovene)"
	description = "A small bottle containing Dylovene pills that treat toxin damage and poisoning."
...
func init_contents():
	...
	for i in range(10):
		contents.append({
			"name": "Dylovene Pill",
			"heal_toxin": 10.0,
			"description": "A pill that neutralizes toxins and poisons."
		})
...
func use_on(user, target, targeted_limb = ""):
	...
	if health_system:
		# Apply healing
		health_system.adjustToxLoss(-pill.heal_toxin)
	...
	if user == target:
		if user and user.has_method("display_message"):
			user.display_message("You swallow a Dylovene pill. [%d remaining]" % contents.size())
	else:
		if user and user.has_method("display_message"):
			user.display_message("You feed a Dylovene pill to %s. [%d remaining]" % [target.name, contents.size()])
		if target and target.has_method("display_message"):
			target.display_message("%s feeds you a Dylovene pill." % user.name)
