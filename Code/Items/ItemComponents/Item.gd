...
@export var attack_verb: Array[String] = ["hits"]
@export var attack_speed: float = 1.1

@export_group("Equipment Configuration")
@export var equip_slot_flags: int = Slots.LEFT_HAND | Slots.RIGHT_HAND
@export var equip_delay: float = 0.0
@export var unequip_delay: float = 0.0
...
@export_group("Throwing Properties")
@export var throwable: bool = true
@export var throw_accuracy: float = 1.0
@export var throw_damage_multiplier: float = 1.0
...
func _ready():
	super._ready()
	_initialize_item_systems()

func _initialize_item_systems():
	_ensure_interaction_area()
	_ensure_valid_equip_slots()
	_register_item_groups()

func _register_item_groups():
...
func _unequipped_internal(user, slot: int):
	...
	_setup_unequipped_state()
	emit_signal("is_unequipped", user, slot)

func _setup_equipped_state(user):
	if get_parent() != user:
		if get_parent():
...
func _move_to_world(user, drop_pos: Vector2):
	var world = user.get_parent()
	if get_parent() != world:
		if get_parent():
			get_parent().remove_child(self)
...
func interact(user) -> bool:
	...
	if pickupable and not has_flag(item_flags, ItemFlags.IN_INVENTORY):
		return _attempt_pickup(user)
	
	return false

func _attempt_pickup(user) -> bool:
...
func get_examine_text(examiner = null) -> String:
	var base_text = description if description != "" else "This is " + get_display_name() + "."
	
	if w_class > 0:
		var size_descriptions = ["tiny", "small", "normal", "bulky", "huge"]
		var size_index = min(w_class - 1, size_descriptions.size() - 1)
		base_text += " It looks " + size_descriptions[size_index] + "."
	
	if sharp:
		base_text += " It has a sharp edge."
	
	if force > 0:
		base_text += " It could be used as a weapon."
...
func attack(target, user):
	...
	damage *= throw_damage_multiplier
	
	if "take_damage" in target:
		target.take_damage(damage, 1, "melee", true, 0.0, user)
...
func can_use() -> bool:
	return not is_in_group("depleted_items")
...
func _throw_to_position_internal(thrower, start_pos: Vector2, target_position: Vector2):
	...
	_play_throw_audio()
	await _animate_throw(final_position)
	_on_throw_complete()
...
func calculate_landing_position(target_position: Vector2) -> Vector2:
	var accuracy = throw_accuracy / (1.0 + (w_class - 1) * 0.3)
...
func _animate_throw(final_position: Vector2):
				...
				if entity != self and "entity_type" in entity:
					if entity.entity_type == "character" or entity.entity_type == "mob":
						if entity.has_method("take_damage"):
							var throw_damage = max(force * throw_damage_multiplier, 2.0)
							entity.take_damage(throw_damage, 1, "blunt", true, 0.0, self)
...
func _play_throw_audio():
	if throw_sound:
		...
		var audio_manager = get_node_or_null("/root/AudioManager")
		if audio_manager:
			audio_manager.play_positioned_sound("throw", global_position, 0.4)
...
func _destroy_item_local():
	if inventory_owner:
		var inventory_system = inventory_owner.get_node_or_null("InventorySystem")
		if inventory_system:
			var slot = inventory_system.find_slot_with_item(self) if inventory_system.has_method("find_slot_with_item") else 0
			if slot != 0:
				inventory_system.equipped_items[slot] = null
				inventory_system.remove_from_lists(self) if inventory_system.has_method("remove_from_lists") else null
				inventory_system.update_slot_visuals(slot) if inventory_system.has_method("update_slot_visuals") else null
				inventory_system.emit_signal("inventory_updated") if inventory_system.has_signal("inventory_updated") else null
...
func deserialize(data: Dictionary):
	...
	if "throw_accuracy" in data: throw_accuracy = data.throw_accuracy
	if "throw_damage_multiplier" in data: throw_damage_multiplier = data.throw_damage_multiplier
	if "throwable" in data: throwable = data.throwable
