...
func initiate_fireman_carry_sequence(target: Node) -> bool:
	...
	var carry_time = max(1.0, 5.0 - (skill_level * 0.5))
	
	show_message("You begin lifting " + _get_entity_name(target) + " into a fireman's carry...")
	
	var timer = get_tree().create_timer(carry_time)
	await timer.timeout
