...
func _check_door_collision(target_tile: Vector2i, z_level: int) -> int:
	...
	var door_integration = world.get_node_or_null("WorldDoorIntegration")
	if not door_integration:
		return CollisionType.NONE
	
	var door_info = door_integration.get_door_collision_info(target_tile, z_level)
	
	if not door_info.has_doors or not door_info.blocked:
		return CollisionType.NONE
	
	var blocking_door = door_info.doors[0] as Door
	if blocking_door:
		set_meta("last_blocking_door", blocking_door)
		
		if door_info.can_auto_open and blocking_door._check_access(controller):
			call_deferred("_try_auto_open_door", blocking_door)
			return CollisionType.NONE
...
func _handle_door_collision_effects(target_tile: Vector2i, direction: Vector2i):
	var blocking_door = get_meta("last_blocking_door", null) as Door
	
	if not blocking_door or not is_instance_valid(blocking_door):
		return
	
	if audio_system:
		if blocking_door._check_access(controller):
	...
	if blocking_door.can_be_bumped:
		blocking_door.on_bump(controller)
	
	if not blocking_door._check_access(controller):
		if sensory_system:
...
func _handle_drift_tile_change(new_tile_pos: Vector2i):
	...
	if _is_valid_tile(new_tile_pos) and can_entity_move_to(controller, new_tile_pos, z_level):
		...
		if door and door._check_access(controller):
...
func _apply_state_effects(old_state: int, new_state: int):
	# Reset speed modifier when leaving crawling
...
func _get_door_at_tile(tile_pos: Vector2i, z_level: int) -> Door:
	if not world:
		return null
	...
	return door_integration.get_door_at_tile(tile_pos, z_level)
...
func can_entity_move_to(entity, tile_pos: Vector2i, z_level: int) -> bool:
	...
	if world:
		var door_integration = world.get_node_or_null("WorldDoorIntegration")
		if door_integration:
			return door_integration.can_entity_move_to_tile(entity, tile_pos, z_level)
