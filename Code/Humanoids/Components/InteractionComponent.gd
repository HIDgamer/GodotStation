...
func _handle_intent_interaction(target: Node) -> bool:
	...
	match current_intent:
		...
		CurrentIntent.DISARM:
			return await _handle_disarm_interaction(target, active_item)
		CurrentIntent.GRAB:
			return await _handle_grab_interaction(target, active_item)
		CurrentIntent.HARM:
...
func _handle_help_interaction(target: Node, active_item: Node) -> bool:
	...
	if _can_fireman_carry_target(target):
		if controller.grab_pull_component and controller.grab_pull_component.grab_state == controller.grab_pull_component.GrabState.AGGRESSIVE:
			return await _initiate_fireman_carry_sequence(target)
		else:
			return _attempt_fireman_carry(target)
	
	if _is_cpr_target(target):
		return await _attempt_cpr(target)
...
func _handle_disarm_interaction(target: Node, active_item: Node) -> bool:
	...
	return await _attempt_disarm_with_do_after(target)
...
func _handle_grab_interaction(target: Node, active_item: Node) -> bool:
	...
	if controller.has_node("GrabPullComponent"):
		...
		if grab_component.grabbing_entity == target:
			return await _upgrade_grab_with_do_after()
...
#region DO_AFTER INTEGRATION
func _attempt_cpr(target: Node) -> bool:
	if not do_after_component:
		show_message("You perform CPR on " + _get_target_name(target) + ".")
		return _execute_cpr(target)
	
	var callback = Callable(self, "_execute_cpr")
	return do_after_component.start_medical_action("cpr", target, callback)

func _execute_cpr(target: Node) -> bool:
	if not target or not is_instance_valid(target):
		return false
	
	if not _is_adjacent_to(target):
		show_message("You need to be closer to perform CPR!")
		return false
	
	if target.has_method("receive_cpr"):
		return target.receive_cpr(controller)
	else:
		show_message("You perform CPR on " + _get_target_name(target) + ".")
		return true

func _initiate_fireman_carry_sequence(target: Node) -> bool:
	if not do_after_component:
		return controller.grab_pull_component.initiate_fireman_carry_sequence(target)
...
func _execute_fireman_carry(target: Node) -> bool:
	...
	if controller.has_node("GrabPullComponent"):
		...
		if grab_component.grabbing_entity == target:
			...
			show_message("You successfully lift " + _get_target_name(target) + " into a fireman's carry!")
			return true
	
	return false

func _upgrade_grab_with_do_after() -> bool:
	if not do_after_component:
		if controller.has_node("GrabPullComponent"):
			return controller.get_node("GrabPullComponent").upgrade_grab()
		return false
	
	var callback = Callable(self, "_execute_grab_upgrade")
	return do_after_component.start_grab_upgrade_action(callback)

func _execute_grab_upgrade() -> bool:
	if controller.has_node("GrabPullComponent"):
		return controller.get_node("GrabPullComponent").upgrade_grab()
	return false

func _attempt_disarm_with_do_after(target: Node) -> bool:
	if not do_after_component:
		return _attempt_disarm_immediate(target)
	
	var callback = Callable(self, "_execute_disarm")
	return do_after_component.start_combat_action("disarm_attempt", target, callback)

func _execute_disarm(target: Node) -> bool:
	if not target or not is_instance_valid(target):
		return false
	
	return _attempt_disarm_immediate(target)

func _attempt_disarm_immediate(target: Node) -> bool:
...
func _use_item_on_target(item: Node, target: Node) -> bool:
	...
	if _should_use_do_after_for_item(item):
		if not do_after_component:
			return await _execute_item_use(item, target)
		
		var callback = Callable(self, "_execute_item_use")
		return do_after_component.start_item_action("use_complex_item", item, callback.bind(item, target))
	else:
		return await _execute_item_use(item, target)

func _execute_item_use(item: Node, target: Node) -> bool:
	var result = false
...
func _should_use_do_after_for_item(item: Node) -> bool:
	if not item:
		return false
...
func _is_cpr_target(target: Node) -> bool:
	if not target or target == controller:
		return false
	...
	if "is_unconscious" in target and target.is_unconscious:
		if "cardiac_arrest" in target and target.cardiac_arrest:
			return true
