...
func try_reload_weapon(weapon: Node, magazine: Node) -> bool:
	...
	if do_after_component and do_after_component.is_performing_action():
		_show_message("You're busy doing something else!")
		return false
	
	# Check if reload should use do_after
	if _should_use_do_after_for_reload(weapon):
		if not do_after_component:
			return _execute_reload(weapon, magazine)
		
		var callback = Callable(self, "_execute_reload")
		return do_after_component.start_weapon_action("reload_weapon", weapon, callback.bind(weapon, magazine))
	else:
		return _execute_reload(weapon, magazine)

func _execute_reload(weapon: Node, magazine: Node) -> bool:
	if not weapon or not magazine or not is_instance_valid(weapon) or not is_instance_valid(magazine):
		return false
...
func _should_use_do_after_for_reload(weapon: Node) -> bool:
	if not weapon:
		return false
	
	var weapon_type = _get_weapon_type(weapon)
	
	# Complex weapons should use do_after
	if weapon_type in ["specialist", "heavy"]:
		return true
	
	# Large weapons should use do_after
	if "w_class" in weapon and weapon.w_class > 4:
		return true
	
	# Weapons with explicit reload time
	if "reload_time" in weapon and weapon.reload_time > 1.0:
		return true
...
func eject_magazine_to_hand(weapon: Node) -> bool:
	...
	# Check if ejection should use do_after
	if _should_use_do_after_for_magazine_operation(weapon):
		if not do_after_component:
			return _execute_magazine_eject_to_hand(weapon)
		
		var callback = Callable(self, "_execute_magazine_eject_to_hand")
		var config_override = {
			"base_duration": 1.0,
			"display_name": "ejecting magazine from " + _get_weapon_name(weapon)
		}
		return do_after_component.start_action("eject_magazine", config_override, callback, weapon)
	else:
		return _execute_magazine_eject_to_hand(weapon)

func _execute_magazine_eject_to_hand(weapon: Node) -> bool:
	if not weapon or not is_instance_valid(weapon) or not inventory_system:
		return false
...
func eject_magazine_to_floor(weapon: Node) -> bool:
	...
	if _should_use_do_after_for_magazine_operation(weapon):
		if not do_after_component:
			return _execute_magazine_eject_to_floor(weapon)
		
		var callback = Callable(self, "_execute_magazine_eject_to_floor")
		var config_override = {
			"base_duration": 0.8,
			"display_name": "ejecting magazine from " + _get_weapon_name(weapon)
		}
...
func _execute_magazine_eject_to_floor(weapon: Node) -> bool:
...
func _should_use_do_after_for_magazine_operation(weapon: Node) -> bool:
	if not weapon:
		return false
	...
	# Weapons with complex magazines
	if "complex_magazine" in weapon and weapon.complex_magazine:
		return true
...
func chamber_round(weapon: Node) -> bool:
	...
	# Check if chambering should use do_after
	if _should_use_do_after_for_chamber(weapon):
		if not do_after_component:
			return _execute_chamber_round(weapon)
		
		var callback = Callable(self, "_execute_chamber_round")
		return do_after_component.start_weapon_action("chamber_round", weapon, callback)
	else:
		return _execute_chamber_round(weapon)

func _execute_chamber_round(weapon: Node) -> bool:
	if not weapon or not is_instance_valid(weapon):
		return false
...
func _should_use_do_after_for_chamber(weapon: Node) -> bool:
	if not weapon:
		return false
...
func cycle_firing_mode(weapon: Node) -> bool:
	...
	# Check if mode switching should use do_after
	if _should_use_do_after_for_mode_switch(weapon):
		if not do_after_component:
			return _execute_firing_mode_cycle(weapon)
		
		var callback = Callable(self, "_execute_firing_mode_cycle")
		var config_override = {
			"base_duration": 1.5,
			"display_name": "switching firing mode on " + _get_weapon_name(weapon)
		}
		return do_after_component.start_action("switch_firing_mode", config_override, callback, weapon)
	else:
		return _execute_firing_mode_cycle(weapon)

func _execute_firing_mode_cycle(weapon: Node) -> bool:
	if not weapon or not is_instance_valid(weapon):
		return false
...
func _should_use_do_after_for_mode_switch(weapon: Node) -> bool:
	if not weapon:
		return false
	...
	# Weapons with complex modes
	if "complex_fire_modes" in weapon and weapon.complex_fire_modes:
		return true
...
func toggle_weapon_safety(weapon: Node) -> bool:
	...
	# Check if safety toggle should use do_after
	if _should_use_do_after_for_safety(weapon):
		if not do_after_component:
			return _execute_safety_toggle(weapon)
		
		var callback = Callable(self, "_execute_safety_toggle")
		var config_override = {
			"base_duration": 0.8,
			"display_name": "toggling safety on " + _get_weapon_name(weapon)
		}
		return do_after_component.start_action("toggle_safety", config_override, callback, weapon)
	else:
		return _execute_safety_toggle(weapon)

func _execute_safety_toggle(weapon: Node) -> bool:
	if not weapon or not is_instance_valid(weapon):
		return false
...
func _should_use_do_after_for_safety(weapon: Node) -> bool:
	if not weapon:
		return false
