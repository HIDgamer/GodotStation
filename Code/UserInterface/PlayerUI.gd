...
const STORAGE_SLOT_SIZE: int = 32
...
var all_slots: Array[String] = [
	"LeftHandSlot", "RightHandSlot", "HeadSlot", "EyesSlot", "BackSlot", 
	"MaskSlot", "UniformSlot", "ArmorSlot", "Ear1Slot", "Ear2Slot", 
	"GlovesSlot", "ShoesSlot", "IDSlot", "BeltSlot", "Pouch1", "Pouch2", "SuitSlot"
]
...
var current_open_storage: Node = null
...
func handle_mouse_motion(event: InputEventMouseMotion):
	if is_mouse_pressed and not is_dragging:
		var distance = mouse_press_position.distance_to(event.position)
		if distance > DRAG_THRESHOLD:
			is_dragging = true
			if dragging_item:
				setup_drag_preview(dragging_item)

func close_all_open_menus():
	close_equipment_menu()
	close_current_storage()

func close_equipment_menu():
	var equipment_button = $Control/EquipmentItems/EquipmentButton
	var equipment_menu = $Control/EquipmentItems/EquipmentButton/EquipmentSlots
	
	if equipment_button and equipment_button.button_pressed:
		equipment_button.button_pressed = false
	
	if equipment_menu and equipment_menu.visible:
		equipment_menu.visible = false

func close_current_storage():
	if current_open_storage and current_open_storage.has_method("close_storage"):
		current_open_storage.close_storage(entity)
...
func is_click_in_storage_panel(position: Vector2) -> bool:
	if current_open_storage and current_open_storage in storage_panels:
		var panel = storage_panels[current_open_storage]
...
func handle_storage_panel_input(event: InputEventMouseButton):
	if not current_open_storage or current_open_storage not in storage_panels:
		return
	
	var panel = storage_panels[current_open_storage]
	if not panel or not panel.visible or not panel.get_global_rect().has_point(event.position):
		return
	
	if event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			handle_storage_panel_press(current_open_storage, panel, event)
		else:
			handle_storage_panel_release(current_open_storage, panel, event)
	elif event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
		handle_storage_panel_right_click(current_open_storage, panel, event)
	elif event.button_index == MOUSE_BUTTON_MIDDLE and event.pressed:
		handle_storage_panel_middle_click(current_open_storage, panel, event)

func handle_storage_panel_press(storage_item: Node, panel: Control, event: InputEventMouseButton):
	...
	if clicked_item and is_storage_item(clicked_item):
		dragging_item = clicked_item
		dragging_from_slot = find_slot_name_for_item(clicked_item)

func handle_storage_panel_release(storage_item: Node, panel: Control, event: InputEventMouseButton):
	is_mouse_pressed = false
	
	if is_dragging and dragging_item:
		handle_storage_item_drag_release(event.position)
	else:
...
func handle_storage_item_drag_release(release_position: Vector2):
	var target_slot = find_target_slot(release_position)
	
	if target_slot in ["LeftHandSlot", "RightHandSlot"]:
		var slot_id = slot_mapping[target_slot]
		var active_item = inventory_system.get_active_item()
		
		if not active_item:
			var current_slot = find_slot_for_item(dragging_item)
			if current_slot != -1:
				var unequip_result = inventory_system.unequip_item(current_slot)
				if unequip_result == inventory_system.ITEM_UNEQUIP_UNEQUIPPED:
					if inventory_system.equip_item(dragging_item, slot_id):
						play_sound_effect(SoundEffect.EQUIP)
					else:
						inventory_system.equip_item(dragging_item, current_slot)
...
func handle_storage_panel_click(storage_item: Node, panel: Control, event: InputEventMouseButton):
	...
	if clicked_item:
		if is_storage_item(clicked_item):
			handle_storage_item_click_action(clicked_item)
		else:
			if not held_item:
				retrieve_item_from_storage(storage_item, clicked_item)
			else:
				put_item_in_storage(storage_item, held_item)
	else:
		if held_item:
			put_item_in_storage(storage_item, held_item)

func handle_storage_item_click_action(storage_item: Node):
	var held_item = inventory_system.get_active_item()
...
func find_slot_name_for_item(item: Node) -> String:
	if not inventory_system:
		return ""
	
	for slot_name in slot_mapping:
		var slot_id = slot_mapping[slot_name]
		var slot_item = inventory_system.get_item_in_slot(slot_id)
		if slot_item == item:
			return slot_name
	
	return ""

func find_slot_for_item(item: Node) -> int:
	if not inventory_system:
		return -1
...
func retrieve_item_from_storage(storage_item: Node, clicked_item: Node):
	var success = false
...
func put_item_in_storage(storage_item: Node, held_item: Node):
	if inventory_system.interact_with_storage_item(storage_item, held_item):
		update_storage_panel(storage_item)
		play_sound_effect(SoundEffect.CLICK)
	else:
...
func handle_storage_panel_right_click(storage_item: Node, panel: Control, event: InputEventMouseButton):
	var clicked_item = find_storage_item_at_position(storage_item, panel, event.position)
	if clicked_item:
		emit_signal("item_right_clicked", clicked_item)
		play_sound_effect(SoundEffect.CLICK)

func handle_storage_panel_middle_click(storage_item: Node, panel: Control, event: InputEventMouseButton):
	var clicked_item = find_storage_item_at_position(storage_item, panel, event.position)
	if clicked_item:
...
func update_storage_panel(storage_item: Node):
	...
	var storage_items = storage_item.storage_items if "storage_items" in storage_item else []
	var max_capacity = get_storage_max_capacity(storage_item)
	
	var panel_size = calculate_storage_panel_size(max_capacity)
	panel.custom_minimum_size = panel_size
	panel.size = panel_size
	
	header.position = Vector2(STORAGE_PANEL_PADDING, STORAGE_PANEL_PADDING)
	header.size = Vector2(panel_size.x - 2 * STORAGE_PANEL_PADDING, 20)
	
	content_area.position = Vector2(STORAGE_PANEL_PADDING, 30)
	content_area.size = Vector2(panel_size.x - 2 * STORAGE_PANEL_PADDING, STORAGE_SLOT_SIZE)
	
	create_horizontal_storage_display(storage_items, max_capacity, content_area)

func get_storage_max_capacity(storage_item: Node) -> int:
	if "storage_max_size" in storage_item:
		return storage_item.storage_max_size
	elif "storage_slots" in storage_item:
		return storage_item.storage_slots
	else:
		return 6

func calculate_storage_panel_size(max_capacity: int) -> Vector2:
	var width = max_capacity * (STORAGE_SLOT_SIZE + 4) - 4 + 2 * STORAGE_PANEL_PADDING
	var height = STORAGE_SLOT_SIZE + 2 * STORAGE_PANEL_PADDING + 30
	
	return Vector2(width, height)

func create_horizontal_storage_display(storage_items: Array, max_capacity: int, content_area: Control):
	for i in range(max_capacity):
		var x = i * (STORAGE_SLOT_SIZE + 4)
		var y = 0
		
		var slot_bg = Panel.new()
		slot_bg.position = Vector2(x, y)
		slot_bg.size = Vector2(STORAGE_SLOT_SIZE, STORAGE_SLOT_SIZE)
		...
		if i < storage_items.size():
			...
			var item_container = create_storage_item_display(item, Vector2(STORAGE_SLOT_SIZE, STORAGE_SLOT_SIZE))
...
func show_storage_panel(storage_item: Node):
	...
	current_open_storage = storage_item
...
func hide_storage_panel(storage_item: Node):
	...
	if current_open_storage == storage_item:
		current_open_storage = null
...
func handle_shift_item_click(item: Node, slot_id: int):
	...
	if slot_name not in ["LeftHandSlot", "RightHandSlot"]:
...
func handle_drag_release(position: Vector2):
	...
	if current_open_storage and current_open_storage in storage_panels:
		var panel = storage_panels[current_open_storage]
		if panel and panel.visible and panel.get_global_rect().has_point(position):
			if inventory_system.interact_with_storage_item(current_open_storage, dragging_item):
				update_storage_panel(current_open_storage)
				play_sound_effect(SoundEffect.CLICK)
				return
			else:
				play_sound_effect(SoundEffect.ERROR)
...
func validate_slot_interaction(slot_name: String) -> bool:
	if slot_name in ["HeadSlot", "EyesSlot", "MaskSlot", "UniformSlot", "ArmorSlot", "GlovesSlot", "ShoesSlot", "SuitSlot", "Ear1Slot", "Ear2Slot"]:
		var equipment_menu = $Control/EquipmentItems/EquipmentButton/EquipmentSlots
...
func is_position_in_ui(position: Vector2) -> bool:
	...
	var equipment_menu = $Control/EquipmentItems/EquipmentButton/EquipmentSlots
	if equipment_menu and equipment_menu.visible and equipment_menu.get_global_rect().has_point(position):
		return true
	
	if current_open_storage and current_open_storage in storage_panels:
		var panel = storage_panels[current_open_storage]
		if panel and panel.get_global_rect().has_point(position):
			return true
