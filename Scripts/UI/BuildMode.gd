...
# GameManager (the old prototype's god-object) no longer exists as an
# autoload - Communications.gd replaced it with the current architecture's
# own managers. Looked up defensively, same pattern as AdminSpawnPopup.gd,
# rather than referenced as a bare global: it's genuinely not there today.
...
func _ready() -> void:
	...
	if not _game_manager:
		print("[BuildMode] GameManager not available - build-action signal relay disabled.")
...
func _connect_build_signal(signal_names: Array) -> void:
	...
	for signal_name in signal_names:
		if _game_manager.has_signal(signal_name):
			_game_manager.connect(signal_name, Callable(self, "_on_build_action_received"))
			return
...
func _broadcast_build_action(action: String, data: Dictionary) -> void:
	...
	if multiplayer.is_server():
		_game_manager.call("SendBuildAction", multiplayer.get_unique_id(), action, data)
