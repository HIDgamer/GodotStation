...
1. **Networking / replication** — generalize the proven `PlayerMob.cs` client-request/server-authority-broadcast pattern so turfs, dropped items, and (once they exist) doors replicate to every client, not just the player's own mob's state. This is extending a working pattern, not building networking from scratch. **Status: item ground-state replication done (2026-09-14) — see below. Turf-mutation replication deliberately deferred until Phase 2/3 build an actual turf-mutation feature to wire it into (no untested plumbing with zero callers).**
   ...
   **What this means for signing off Phase 1:** the replication *code* is done and individually verified (compiles clean, every autoload/script initializes correctly in isolation, matches established RPC conventions exactly). The specific cross-peer *behavior* claim (does peer B actually see peer A's dropped item, does a late joiner actually get caught up) has **not** been automatically verified end-to-end, and is not yet manually verified either. Before treating Phase 1 as complete: either (a) manually run two real game windows (not headless) side by side and confirm visually, or (b) retry the automated harness on a different machine/CI runner where this environmental issue may not reproduce (it may well be specific to this dev machine's antivirus/runtime setup), or (c) investigate the .NET/IPC angle properly as its own task. Do not claim Phase 1 verified without one of these actually happening.
