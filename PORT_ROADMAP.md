...
3. **Doors/airlocks** — brand new system: base door class, then full airlock parity (wire panel, dual power rails, bolts, damage/forced-entry, tool interactions). Largest near-term phase by DM line count (~5,550 lines of source behavior). **Status: base door class done (2026-09-14) — see below. Airlock-specific complexity (wire panel/power/bolts/tool interactions) not started.**
   ...
   **Deliberately deferred to the airlock-specific stage:** wire panel, power rails, bolts/forced-entry, tool interactions (welding, hacking), and a manual player-triggered close (today only bump-to-open + auto-close exist - a generic click/interact dispatch is Phase 4's job, not this stage's).
